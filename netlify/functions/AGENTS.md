...
| `create-authnet-transaction.js` | POST, OPTIONS | Charges a card via Authorize.Net using an Accept.js opaque token (today's installment); server computes price from `pricing.json`; on approval also creates the CIM profile and — only for products `pricing.json` → `billing` marks as subscriptions (`lib/billing-model.js`; not every product is one) and plans ≤ 12 months (ARB interval cap) — a recurring Authorize.Net subscription (ARB) at the plan's own cadence and full price. One-time products are charged once for the full term. Fires post-charge conversion tracking. Rate-limited (10/min/IP). | Authorize.Net API |
...
| `getBillingHistory.js` | POST, OPTIONS | Auth'd: fetches a patient's payment methods + charge history (Supabase `funnel_orders`, plus legacy Stripe if still configured) by the verified session email — now also returns `authnet_subscription_id`/`subscription_status` per charge so the Hub can show and cancel an active subscription. | Supabase (auth) + Stripe |
...
| `stripeWebhook.js` | POST | Inbound Stripe webhook (signature verified via `STRIPE_WEBHOOK_SECRET`). Handles `payment_intent.succeeded` (fires Meta CAPI/GA4 conversion + internal notify), `payment_intent.payment_failed` (+ patient email), `charge.dispute.created`, `charge.refunded` (+ patient email), `payment_intent.requires_action`. | Stripe + Resend + n8n |
...
| Directory | Purpose |
|-----------|---------|
| `lib/` | Shared utility modules (MDI client, Supabase/Firebase auth verification, PHI encryption, rate limiting, product catalog, quiz validation, conversion tracking, structured logging) plus the email system: `lib/email/` (the send/journey engine) and `lib/email-templates/` (every rendered email). See `lib/AGENTS.md`. |
...
needed" email was silently dropped after logging. See `docs/EMAIL_FLOWS.md`
for the full catalog, triggers, timing, required env vars, and the manual
setup checklist (Resend domain verification, webhook registration,
Authorize.Net subscription events). `lib/email/AGENTS.md`-equivalent detail
lives in `lib/AGENTS.md`'s own manual section.
