...
- MDI product/questionnaire/offering IDs are centralized in `lib/products.js`; functions never hardcode them. Same rule for the other MDI-side ids/config touched by this session's work — `MDI_PARTNER_ID`, `MDI_SANDBOX_ENV_ID`/`MDI_LIVE_ENV_ID` (`lib/mdi-voucher.js`), `MDI_TEST_TAG_KEY`/`MDI_TEST_TAG_NAME`/`MDI_TEST_TAG_COLOR` (`lib/mdi-tags.js`), `MDI_APPROVAL_CHECK_STATUSES` (`checkAdditionalApprovals.js`) — all read from env with a same-value fallback default, never a bare literal in the hot path. See the env var block at the top of `netlify.toml`.
- MDI test-vs-live is decided in one place, `lib/mdi-voucher.js` (`resolveTestMode`), shared by `submitQuiz.js` and `retryPendingCases.js`. **Safe by default:** vouchers are test vouchers (`demo: true` + `metadata: "TEST CASE | …"`) unless `MDI_LIVE_MODE=true` **and** `MDI_ALLOW_LIVE_ORDERS=true`. `MDI_FORCE_TEST=true` is the kill switch; `MDI_TEST_EMAIL_PATTERNS` keeps QA emails as tests; `MDI_TEST_FULL_FLOW=true` creates a real (tagged) case instead of a demo voucher, and `mdiWebhook.js` attaches the global `test-case` tag via `lib/mdi-tags.js`. See `docs/MDI_TESTING.md`.
- "Approved (Action Required)" per MDI's go-live guidance is **not** the `case_approved` webhook event handled in `mdiWebhook.js` (that's a separately-documented "clinician gave final approval" concept we could not confirm is the same thing) — it's the documented `is_additional_approval_needed` case flag, polled hourly by `checkAdditionalApprovals.js` via `POST /v1/partner/cases/status/:status`.
