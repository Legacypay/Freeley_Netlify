...
`docs/HANDOFF_2026-09-01.md` is the authoritative record of the pre-launch work: what shipped, decisions already made (renewal pricing, 24-month plan, per-product billing default, fail-closed payment verification), environment facts that are not derivable from code (which Authorize.Net credentials still work, which APIs are enabled where, which Netlify env vars are missing), and the open list split into "needs the user/client" vs "code not started". `docs/SECURITY_AUDIT_2026-09-01.md` holds the audit table. Read both before touching billing, intake, or the Hub — and never probe production intake with a non-`+test@` email.
