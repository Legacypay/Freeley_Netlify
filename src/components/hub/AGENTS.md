...
| File | Description |
|------|-------------|
| `HubAuthScreen.astro` | Sign-in/sign-up form (`#auth-screen`, email/password only — no OAuth, removed 2026-09-01), forgot-password link, show/hide toggle on every password field. Toggles between sign-in/sign-up copy client-side. Calls `signUp`/`signIn`/`resetPassword` from `src/lib/hub/auth.ts`. Hiding this screen and showing the dashboard on success is page-level orchestration in `hub.astro` (listens for the `hub:auth` event), not this component's job. |
