...
> **2026-08-25 update:** this checklist and `docs/MDI_TESTING.md` were written
> independently (two sessions working the same go-live message) and have since
> been reconciled in code. See the inline notes below for what changed;
> `docs/MDI_TESTING.md` is the more detailed reference for the test/live
> decision and env vars.
...
`offerings: [{ "id": "<offering_id>" }]`.~~ **Fixed 2026-08-25** —
`lib/mdi-voucher.js`'s `buildVoucherPayload()` sends `offerings: [{ id }]`;
verified against the real API (Sandbox credentials).
...
- [x] **Added 2026-08-25, without waiting on the Slack confirmation above:**
      `checkAdditionalApprovals.js` (hourly cron) independently polls the
      *documented* `is_additional_approval_needed` case flag via
      `POST /v1/partner/cases/status/:status` and alerts once per case
      (Encounter ID only). This doesn't answer whether `case_approved` ↔
      "Approved" — it sidesteps needing that answer, using the one mechanism
      the public docs confirm unambiguously as a safety net regardless of what
      `case_approved` turns out to mean.
...
~~Deliberately NOT built (YAGNI while tests never reach live): auto-attaching
the "test case" tag... and sending `demo: true` on the voucher.~~ **Both built
2026-08-25** — `lib/mdi-voucher.js` sends `demo: true` by default (MDI:
"Demo vouchers will not create any patient or cases and will not expire"),
and `lib/mdi-tags.js` + `mdiWebhook.js`/`checkAdditionalApprovals.js` create
and auto-attach a global `test-case` tag for full-flow (`MDI_TEST_FULL_FLOW=true`)
test orders that do create a real case. See `docs/MDI_TESTING.md` for the full
decision matrix — the `resolveTestMode()` gate now also accepts this
checklist's `is_test: true` body flag as one of its signals.
